...
def add_new_user(user):
    with get_db_session() as session:
        ...
        try:
            city_from_db = session.query(City).filter_by(City.name == user.city).first()
...
def add_new_object(object):
    with get_db_session() as session:
        created_at = datetime.datetime.now()
        try:
            new_object = Object(name=object.name, url = object.url, date = created_at)
            session.add(new_object)
            session.commit()
            logger.success('Successfully inserted object into db')
        except Exception as e:
            session.rollback()
            logger.error(f'Failed to insert new object into db with error: {e}')
            return False